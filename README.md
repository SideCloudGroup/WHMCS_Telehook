...
Telehook 是一个 WHMCS 插件。客户在客户区绑定一次 Telegram 之后，这个账号下的产品开通、续费账单、催缴和暂停都会发到 Telegram。
...
- **账单**：WHMCS 自动生成续费账单时发送。提前几天出账，沿用 WHMCS 自己的账单设置。下单时的那张账单不发。
- **催缴**：WHMCS 发出账单催缴时同步发送，包括即将到期和逾期提醒。WHMCS 里关掉催缴，这里也不发。
- **暂停**：产品被暂停时发送，包括后台手动暂停和到期自动暂停。

插件只发消息，不会暂停产品，也不会改账单。

## 发送失败

Bot 请求失败时只记入 WHMCS 活动日志，不影响开通、出账、催缴和暂停。
